...
const ContactPage: React.FC = () => {
  ...
  useGSAP(() => {
    ...
    const moveButton = (e: MouseEvent) => {
      ...
      const { left, top, width, height } = (buttonRef.current as HTMLElement).getBoundingClientRect();
  ...
  return (
    <div ref={container} className="min-h-screen bg-[#1C1D20] text-white px-[4%] pt-24 pb-10">
      <div className="max-w-[1440px] mx-auto flex flex-col md:flex-row justify-between gap-12">
        ...
        <div ref={formRef} className="flex-[1.5]">
          ...
          <form className="space-y-8">
            {[
              ...
              { id: "02", label: "What's your email?", placeholder: "name@mail.com" },
              { id: "03", label: "What's the name of your organization?", placeholder: "Write Here" },
              { id: "04", label: "What services are you looking for?", placeholder: "Web Design, Web Development..." },
              { id: "05", label: "Your message", placeholder: "Write Messge here..." },
            ].map((field) => (
              <div key={field.id} className="group border-t border-white/10 pt-6 relative">
                <div className="flex items-start gap-4">
                  <span className="text-white/20 text-[10px] mt-2 font-mono">{field.id}</span>
                ...
                <div className="absolute bottom-0 left-0 w-full h-[1px] bg-[#455CE9] scale-x-0 group-focus-within:scale-x-100 transition-transform duration-500 origin-left" />
              </div>
            ))}
          </form>
        </div>
        ...
        <div className="md:w-[300px] flex flex-col items-start pt-4">
          ...
          <div className="mb-10">
             <img src="/images/ugosmile.png" alt="Ugo" className="w-20 h-20 rounded-full object-cover mb-6 border border-white/10 grayscale hover:grayscale-0 transition-all duration-500" />
             ...
             <div className="space-y-6 text-[11px] uppercase tracking-widest text-white/40">
                <section>
                  ...
                  <a href="mailto:info@ugochiori.com" className="text-white block hover:text-[#455CE9] transition-colors">ugochiori@gmail.com</a>
                ...
                <section>
                  ...
                  <div className="flex flex-col gap-2">
                    {['LinkedIn', 'GitHub', 'Twitter'].map(link => (
                      <a key={link} href="#" className="text-white hover:text-[#455CE9] transition-all w-fit cursor-pointer">{link}</a>
                    ))}
                  </div>
                </section>
             </div>
